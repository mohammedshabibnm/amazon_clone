import React from 'react'
import "../App.css"

function Footermethod() {
    return ( <div>

<div className="lastfoot">

<div className="fot">

<div className="fot1">
<h4 className='fowhite'>Get to Know Us</h4>

<a className='linkdecs' href="">About Us</a>
<a className='linkdecs' href="">Careers</a>
<a className='linkdecs' href="">Press Releases</a>
<a className='linkdecs' href="">Amazon Science</a>



</div>

<div className="fot1">
<h4 className='fowhite'>	
Connect with Us</h4>

<a className='linkdecs' href="">Facebook</a>
<a className='linkdecs' href="">Twitter</a>
<a className='linkdecs' href="">Instagram</a>




</div>

<div className="fot1">
<h4  className='fowhite'>	
Make Money with Us</h4>

<a className='linkdecs' href="">Sell on Amazon</a>
<a className='linkdecs' href="">Sell under Amazon Accelerator</a>
<a className='linkdecs' href="">Protect and Build Your Brand</a>
<a className='linkdecs' href="">Amazon Global Selling</a>
<a className='linkdecs' href="">Become an Affiliate</a>
<a className='linkdecs' href="">Fulfilment by Amazon</a>
<a className='linkdecs' href="">Advertise Your Products</a>
<a className='linkdecs' href="">Amazon Pay on Merchants</a>




</div>

<div className="fot1">
<h4 className='fowhite'>	
	
Let Us Help You</h4>

<a className='linkdecs' href="">COVID-19 and Amazon</a>
<a className='linkdecs' href="">Your Account</a>
<a className='linkdecs' href="">Returns Centre</a>
<a className='linkdecs' href="">100% Purchase Protection</a>
<a className='linkdecs' href="">Amazon App Download</a>
<a className='linkdecs' href="">Help</a>




</div>
</div>

</div>

<div className="footlang">

<div className="footlang1">


<div className="footcou">
<img width={90} height={50} src="data:image/png;base64," alt="" />

<select className='aad' name="" id="">
    <option value="">English</option>
<option value="">Malayalam</option>
<option value="">Hindi</option>
<option value="">Tamil</option>
</select>
</div>

<div className="lastdiv">
<span className='upp'>Brazil</span>
<span className='upp'>Australia</span>
<span className='upp'>Canada</span>
<span className='upp'>China</span>
<span className='upp'>France</span>
<span className='upp'>Germany</span>
<span className='upp'>Italy</span>
<span className='upp'>Japan</span>
<span className='upp'>Mexico</span>
<span className='upp'>Netherlands</span>
<span className='upp'>Poland</span>
<span className='upp'>Singapore</span>
<span className='upp'>Spain</span>
<span className='upp'>United Arab Emirates</span>
<span className='upp'>United Kingdom</span>
<span className='upp'>United States</span>



</div>






</div>



</div>




















































































































    </div> );
}

export default Footermethod;